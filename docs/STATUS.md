...
- **`trade_attachments` persistido — CONTRATO PARA FRONTEND:** la tabla existía en el esquema pero nada la usaba (ni en v1 ni acá); además, `listManualTradesWithAnnotationSummary` derivaba `firstAttachmentKey` de `trade_annotations.extra.attachments`, un JSON ad-hoc que nunca llegó a poblarse — **retirado**, ahora lee de `trade_attachments` real. Si algo en el frontend asumía `extra.attachments`, ya no aplica.
  ...
  - Conocido, no arreglado: `deleteAttachment` sólo borra la fila de `trade_attachments`, NO el objeto del bucket (`storage.ts` no tiene una función de borrado). Una captura "borrada" queda guardada en Storage. Necesita `removeAttachmentObject(key)` en `integrations/storage.ts` + llamarla desde `deleteAttachmentAction`.
- **Catálogos sin repetidos + setups globales (2026-09-26) — migraciones `0003`–`0005`, aplicadas a Supabase real:**
  - **Causa de los repetidos:** `UNIQUE (user_id, label)` no frena duplicados cuando `user_id` es NULL (en Postgres NULL ≠ NULL) y el seed hacía `on conflict do nothing`, que nunca disparaba: cada `db:setup` agregaba otra copia de los globales (×4). `0003` (datos): deja UNA fila por etiqueta global (menor `id`; estos catálogos no tienen `created_at`), re-apunta `annotation_confluences`/`annotation_emotional_states` al sobreviviente antes de borrar (`ON CONFLICT DO NOTHING` por la PK compuesta) y borra el resto. `0004` (esquema, generada): las 3 uniques pasan a `UNIQUE NULLS NOT DISTINCT`. Resultado real: confluencias 34→10 (8 globales + 2 propias), estados 33→9 (8 + 1), filas de unión 4 y 2 intactas. Se arregló en los datos, no en la UI.
  - **Seed idempotente:** `setup.ts` ya no acumula. Probado corriendo `db:setup` DOS veces seguidas con conteos: 10/8, 9/8 y 4/3 idénticos tras cada corrida, sin reaplicar migraciones. Además la BD rechaza un duplicado global en los 3 catálogos (probado con `INSERT` revertido).
  - **Setups globales (opción a):** `setups.user_id` ahora es NULLABLE (NULL = global) y `listSetups(db, userId)` devuelve globales + propios, igual que confluencias/estados. `0005` convirtió con `UPDATE` (conserva ids) los 3 iniciales del usuario dev (`FVG + CHoCH`/SMC, `Silver Bullet`/ICT, `Ruptura de rango asiático`/Price Action) en globales; el seed los inserta ya como globales. Verificado: la cuenta real (`aaaffc28…`) ve los 3 + su "SET UP A" (4), y su anotación con setup sigue apuntando bien. Firma sin cambios: `listCatalogs().setups` = `{ id, name, family }[]`. Sin fallbacks en runtime.
  - **`get_advisors` tras el cambio:** sin advisories nuevas por estas migraciones. Quedan 6 INFO de RLS sin políticas (bajaron de 18 por la migración de login), 3 INFO de índices sin uso y 2 WARN de la sesión de login, NO de esto: `current_app_user_id()` es `SECURITY DEFINER` ejecutable por `authenticated`, y "Leaked password protection" desactivado en Auth. La política `own_rows` de `setups` no expone ni deja escribir globales por la Data API (la app no la usa).
  - **Aún posible:** un usuario puede crear un setup/confluencia/estado PROPIO con el mismo nombre que uno global (la unicidad es por `(user_id, label)`); se vería repetido en su lista. No cubierto.
- **Campo "cómo cerró" (`exit_reason`) — CONTRATO PARA FRONTEND (le toca a Frontend ahora):** aditivo, nada se rompe.
  - **Contrato** (`@tradevision/contracts`): `ExitReason = "take_profit" | "break_even" | "stop_loss"` (Zod + tipo) y `ManualTrade.exitReason?: ExitReason | null`.
  - **BD:** enum Postgres `exit_reason` + columna `manual_trades.exit_reason` NULLABLE (migración `0004`; las operaciones anteriores quedan en `null`, no se inventa nada). Solo `manual_trades`: `verified_trades` es append-only y no lo lleva.
  - **Repo:** `createManualTrade(db, userId, { …, exitReason?: ExitReason | null })`; `listManualTrades`, `getTradeWithAnnotation` (`trade.exitReason`) y `loadTradeSet` (`manual[].exitReason`) lo devuelven.
  - **Fachada** (`apps/web/lib/data.ts`, rama real y demo-store): `NewManualTrade.exitReason?: ExitReason | null`; `demo-store` `CoreTrade.exitReason?`. Las operaciones sembradas de la demo no lo traen.
  - **Server Action:** `createManualTradeAction` acepta el campo `exitReason` del `FormData` (`"take_profit" | "break_even" | "stop_loss"`; `""` u omitido → `null`). Es OPCIONAL en el Zod para no romper el formulario actual: **Frontend decide si lo exige** en la UI (Leonardo lo quiere como la pregunta principal al registrar) y, si quiere obligarlo, cambia ese Zod a requerido.
  - Probado contra Supabase real: los tres valores + omitido se guardan y vuelven por `createManualTrade`/`listManualTrades`/`loadTradeSet`, y la BD rechaza un valor fuera del enum.
